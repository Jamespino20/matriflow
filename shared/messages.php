            ...
            <?php else: ?>
                <?php foreach ($contacts as $c): ?>
                    <div class="contact-item <?= $activeChatId === (int)$c['user_id'] ? 'active' : '' ?>" onclick="loadChat(<?= $c['user_id'] ?>, '<?= htmlspecialchars($c['first_name'] . ' ' . $c['last_name']) ?>')">
                        <img class="contact-avatar-img" src="<?= base_url("/public/assets/images/avatars/{$c['user_id']}.png") ?>"
...
<script>
                    ...
                    if (json.ok) {
                        ...
                        (json.users || []).forEach(u => {
                            html += `<div class="search-result-item" onclick="startChat(${u.user_id}, '${u.first_name} ${u.last_name}')" style="display:flex; align-items:center; gap:10px;">
                                <img src="${getBase()}/assets/images/avatars/${u.user_id}.png" 
    ...
    function loadChat(id, name) {
        ...
        if (!contactItem) {
            ...
            contactItem.innerHTML = `
                <img class="contact-avatar-img" src="${getBase()}/assets/images/avatars/${id}.png" 
    ...
    function renderMessages(messages) {
        ...
        messages.forEach(m => {
            ...
            if (m.id && !m.is_temp && m.id > lastMessageId) lastMessageId = m.id;
