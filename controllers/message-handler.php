...
try {
    ...
    if ($action === 'search_users') {
        ...
        $stmt = db()->prepare("SELECT u.user_id, u.first_name, u.last_name, u.role, p.patient_id FROM user u 
    ...
    if ($action === 'search_patients') {
        ...
        $stmt = db()->prepare("SELECT p.patient_id, u.first_name, u.last_name, p.identification_number 
                               FROM patient p
                               JOIN user u ON p.user_id = u.user_id
                               WHERE (u.first_name LIKE :q1 OR u.last_name LIKE :q2 
                                      OR p.identification_number LIKE :q3 
