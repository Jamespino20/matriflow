...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!CSRF::validate($_POST['csrf_token'] ?? null)) {
        ...
        if (!TOTP::verify($secret, $code, 1, 6)) {
            $errors[] = 'Invalid code.';
            AuditLogger::log((int) $u['user_id'], 'user', 'LOGIN', (int) $u['user_id'], '2fa_failed');
        } else {
            Auth::mark2FAVerified();
            AuditLogger::log((int) $u['user_id'], 'user', 'LOGIN', (int) $u['user_id'], '2fa_ok');

            // Ensure account is marked active
            db()->prepare("UPDATE user SET account_status = 'active' WHERE user_id = ?")->execute([$u['user_id']]);
            ...
            <form method="post" action="<?= base_url('/public/verify-2fa.php' . ($isInModal ? '?modal=1' : '')) ?>">
                ...
                <?php if (!$isInModal): ?>
                    <div style="margin-top:24px; display:flex; gap:16px; justify-content:center">
                        <a href="/public/logout.php" style="color:var(--muted); font-size:14px">Logout</a>
                    </div>
                <?php endif; ?>
            </form>
        </div>
