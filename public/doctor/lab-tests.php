...
<!-- Review Modal -->
<div id="modal-lab-review" class="modal-overlay modal-clean-center" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; z-index:10000; background:rgba(0,0,0,0.5); align-items:center; justify-content:center;">
    <div class="modal-card" style="background:var(--surface); width:90%; max-width:600px; border-radius:12px; padding:32px; position:relative;">
        <button type="button" class="modal-close" style="position:absolute; top:16px; right:16px;" onclick="document.getElementById('modal-lab-review').style.display='none'">&times;</button>
        <h3>Review Lab Result</h3>
        <p id="lab-patient-name" style="font-weight: 700; color: var(--primary); margin-top: 10px;"></p>

        <form id="form-lab-review" action="/public/controllers/lab-handler.php" method="POST" style="margin-top: 20px;">
...
<script>
    function openLabModal(test) {
        ...
        document.getElementById('lab-released').value = test.released_to_patient || '0';
        document.getElementById('modal-lab-review').style.display = 'flex';
    }
</script>
<?php
