...
(function () {
    ...
    function clearErrors(form) {
        const existingAlerts = form.querySelectorAll('.alert-danger, .alert-success');
    ...
    document.addEventListener('click', function (e) {
        ...
        if (toggleBtn) {
            ...
            if (input) {
                ...
                // Optional: Toggle icon styling here if needed
            ...
            try {
                ...
                if (contentType && contentType.includes('application/json')) {
                    ...
                    if (data.success) {
                        ...
                        const msg = Array.isArray(data.errors) ? data.errors.join(' ') : (data.errors || 'Login failed.');
                    ...
                    if (data.success) {
                        if (data.redirect) {
                            ...
                            if (loginModal) {
                                ...
                                if (firstRow) firstRow.parentNode.insertBefore(successAlert, firstRow);
                            }
                        }
                    } else {
                        const msg = Array.isArray(data.errors) ? data.errors.join(' ') : (data.errors || 'Registration failed.');
                ...
                if (contentType && contentType.includes('application/json')) {
                    ...
                    if (data.ok) {
                        ...
                        if (data.reset_link) {
                            ...
                            linkDiv.innerHTML = '<a href="' + data.reset_link + '" class="btn-text" style="font-weight:700">Click here to reset password</a> or copy:<br><span style="user-select:all">' + data.reset_link + '</span>';
                        ...
                        showError(forgotForm, data.message || 'Failed to generate reset link.');
                    }
                } else {
                ...
                if (contentType && contentType.includes('application/json')) {
                    ...
                    if (data.ok) {
                        ...
                        if (loginModal) {
                            ...
                            const successAlert = document.createElement('div');
                            successAlert.className = 'alert alert-success';
                            successAlert.textContent = data.message || 'Password updated successfully!';
                            const firstRow = loginModal.querySelector('.form-row');
                            if (firstRow) firstRow.parentNode.insertBefore(successAlert, firstRow);
                        }
                    } else {
    ...
    if (urlParams.get('setup2fa') === '1') {
        ...
        if (m) {
            ...
            if (iframe) iframe.src = getBase() + '/setup-2fa.php?modal=1' + (isNewUser ? '&newuser=1' : '');
            window.history.replaceState({}, document.title, getRoot());
        }
    }
