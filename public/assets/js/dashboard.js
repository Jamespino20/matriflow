  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function showInactivityModal() {
      ...
      if (modal) {
        ...
        // [SECURITY] Prevent escaping via "Back" button
        // Push a state so that hitting back triggers popstate
        history.pushState({ inactivity: true }, null, location.href);
        
        const forceLogout = () => {
          const baseUrl = getConfig("baseUrl") || "/public";
          window.location.replace(baseUrl + "/logout.php?reason=inactivity");
        };
        ...
        // If they try to navigate away by other means, also force it if modal is up
        window.addEventListener('beforeunload', (e) => {
           if (modal.style.display === "flex") {
             // We can't easily force redirect on beforeunload without user interaction, 
             // but popstate + cache control handles most cases.
           }
        });

        // Automatically redirect after another minute if they don't click anything
        setTimeout(() => {
          const baseUrl = getConfig("baseUrl") || "/public";
          window.location.href = baseUrl + "/logout.php?reason=inactivity";
        }, 60000);
      }
    }
