            ...
            <?php else: ?>
                <?php foreach ($patients as $p): ?>
                    <tr>
                        ...
                        <td><?= e($p['contact_number'] ?? 'N/A') ?></td>
                        <td><?= $p['last_visit'] ? date('M j, Y', strtotime($p['last_visit'])) : '<span style="color:var(--text-secondary)">None</span>' ?></td>
                        <td style="text-align: right;">
                            <a class="btn btn-secondary btn-sm" href="<?= base_url('/public/secretary/records.php?patient_id=' . $p['user_id']) ?>">Records</a>
                            <button class="btn btn-outline btn-sm" onclick="openVitalsModal(<?= $p['user_id'] ?>, '<?= e($p['first_name'] . ' ' . $p['last_name']) ?>')">Vitals</button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
...
<div id="modal-add-vitals" class="modal-overlay modal-clean-center">
    <div class="modal-card" style="background:var(--surface); width:90%; max-width:400px; border-radius:12px; padding:32px; position:relative;">
        <button type="button" class="modal-close" style="position:absolute; top:16px; right:16px;" onclick="document.getElementById('modal-add-vitals').classList.remove('show')">&times;</button>
        <h3>Record Vital Signs</h3>
        <p id="vitals-patient-name" style="color:var(--text-secondary); margin-top:-10px; margin-bottom:20px;"></p>

        <form action="<?= base_url('/public/controllers/secretary-handler.php') ?>" method="POST">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; align-items: center; gap: 8px;">
                    <input type="number" name="systolic" placeholder="Sys" style="width: 80px; text-align: center;" min="0" max="300" required>
                    <span>/</span>
                    <input type="number" name="diastolic" placeholder="Dia" style="width: 80px; text-align: center;" min="0" max="200" required>
                </div>
            </div>
            <div class="form-group"><label>Heart Rate (bpm)</label><input type="number" name="heart_rate" min="0" max="300"></div>
            <div class="form-group"><label>Temperature (°C)</label><input type="number" step="0.1" name="temperature_celsius" min="30" max="45"></div>
            <div class="form-group"><label>Weight (kg)</label><input type="number" step="0.1" name="weight_kg" min="0" max="500"></div>
...
<script>
    function openVitalsModal(pid, name) {
        ...
        document.getElementById('modal-add-vitals').classList.add('show');
    }
</script>
<?php
