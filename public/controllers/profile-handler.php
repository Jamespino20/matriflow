...
try {
    ...
    if ($action === 'update_profile') {
        ...
        $stmt = db()->prepare("SELECT user_id FROM user WHERE (username = :u OR email = :e) AND user_id != :id");
        ...
        if ($email !== (string)$u['email']) {
            AuditLogger::log((int)$u['user_id'], 'user', 'UPDATE', (int)$u['user_id'], "Email changed from {$u['email']} to $email");
        ...
        // Update User table
        $stmt = db()->prepare('UPDATE user SET username = :u, email = :e, contact_number = :p, address = :a, dob = :d, emergency_contact_name = :en, emergency_contact_phone = :ep WHERE user_id = :id');
        $stmt->execute([
            ':u' => $username,
            ':e' => $email,
            ':p' => $phone,
            ':a' => $fullAddress,
            ':d' => $dob !== '' ? $dob : (empty($u['dob']) ? null : $u['dob']), // Maintain existing DOB if not changing
            ':en' => $emergencyName,
            ':ep' => $emergencyPhone,
            ':id' => (int) $u['user_id']
        ]);

        // Backward compatibility for patient table
        if ($u['role'] === 'patient') {
            $stmt = db()->prepare('UPDATE patient SET address = :a, city = :c, province = :p WHERE user_id = :id');
            $stmt->execute([':a' => $address, ':c' => $city, ':p' => $province, ':id' => (int) $u['user_id']]);
        }
        ...
        if ($cur !== '' || $new !== '' || $conf !== '') {
            ...
            $stmt = db()->prepare('UPDATE user SET password_hash = :ph WHERE user_id = :id');
            $stmt->execute([':ph' => $h, ':id' => (int) $u['user_id']]);

            AuditLogger::log((int)$u['user_id'], 'user', 'UPDATE', (int)$u['user_id'], "Password changed");
    ...
    if ($action === 'delete_account') {
        ...
        // Soft delete
        $stmt = db()->prepare("UPDATE user SET is_active = 0 WHERE user_id = :id");
