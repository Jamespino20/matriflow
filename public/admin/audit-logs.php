...
$countSql = "SELECT COUNT(*) FROM audit_log a LEFT JOIN users u ON a.user_id = u.user_id $whereClause";
...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        ...
        <div class="badge badge-info"><?= number_format($totalEvents) ?> Events Total</div>
    </div>
    ...
    <form method="GET" style="display:flex; flex-direction:column; gap:12px; margin-bottom:24px; padding:16px; background:var(--surface-light); border-radius:8px; border:1px solid var(--border);">
        <!-- Row 1: Search + Action Dropdown -->
        <div style="display:flex; gap:12px; align-items:center;">
            <div class="search-container" style="flex:1;">
                <span class="material-symbols-outlined search-icon">search</span>
                <input type="text" name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Search logs, tables, changes...">
            </div>
            <div style="min-width:180px;">
                <label style="font-size:11px; font-weight:600; color:var(--text-secondary); display:block; margin-bottom:4px;">ACTION TYPE</label>
                <select name="op" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:6px; background:var(--surface);">
                    ...
                    <option value="LOGOUT" <?= $op === 'LOGOUT' ? 'selected' : '' ?>>LOGOUT</option>
                    <option value="PASSWORD_CHANGE" <?= $op === 'PASSWORD_CHANGE' ? 'selected' : '' ?>>PASSWORD_CHANGE</option>
                    <option value="2FA_ENABLE" <?= $op === '2FA_ENABLE' ? 'selected' : '' ?>>2FA ENABLE</option>
                    <option value="2FA_DISABLE" <?= $op === '2FA_DISABLE' ? 'selected' : '' ?>>2FA DISABLE</option>
                    <option value="2FA_VERIFY" <?= $op === '2FA_VERIFY' ? 'selected' : '' ?>>2FA VERIFY</option>
                </select>
            </div>
        </div>
        <!-- Row 2: Date Ranges + Buttons -->
        <div style="display:flex; gap:12px; align-items:flex-end;">
            <div>
                <label style="font-size:11px; font-weight:600; color:var(--text-secondary); display:block; margin-bottom:4px;">FROM DATE</label>
            ...
            <div>
                <label style="font-size:11px; font-weight:600; color:var(--text-secondary); display:block; margin-bottom:4px;">TO DATE</label>
            ...
            <?php if ($q || $op || $table || $dateFrom || $dateTo): ?>
                <a href="audit-logs.php" class="btn btn-outline">Clear All</a>
            <?php endif; ?>
        </div>
    </form>
                    ...
                    <tr onclick="toggleDetails(<?= $log['audit_log_id'] ?>)" style="cursor: pointer;">
                        <td>
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <span id="arrow-<?= $log['audit_log_id'] ?>" class="material-symbols-outlined" style="font-size: 18px; color: var(--text-secondary); transition: transform 0.2s;">chevron_right</span>
                                <div>
