...
// Support token-based setup for new users from email
if (!empty($_GET['token']) && !empty($_GET['uid'])) {
    $uid = (int) $_GET['uid'];
    $token = (string) $_GET['token'];
    $hash = hash('sha256', $token);

    $stmt = db()->prepare("SELECT * FROM password_reset_tokens WHERE user_id = :uid AND token_hash = :hash AND expires_at > NOW() LIMIT 1");
    $stmt->execute([':uid' => $uid, ':hash' => $hash]);
    $tokenRow = $stmt->fetch();

    if ($tokenRow) {
        $userRow = User::findById($uid);
        if ($userRow && (int)$userRow['is_2fa_enabled'] === 0) {
            Auth::loginUser($userRow); // Log them in to allow setup
            // Delete the token so it's one-time use
            $stmt = db()->prepare("DELETE FROM password_reset_tokens WHERE password_reset_token_id = :id");
            $stmt->execute([':id' => (int)$tokenRow['password_reset_token_id']]);
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!CSRF::validate($_POST['csrf_token'] ?? null)) {
        ...
        if (!TOTP::verify($secret, $code, 1, 6)) {
            ...
            AuditLogger::log($uid, 'users', 'UPDATE', $uid, '2fa_enabled');
        ...
        <div class="modal-body">
            ...
            <form method="post" action="<?= base_url('/public/setup-2fa.php' . ($isInModal ? '?modal=1' : '')) ?>">
                ...
                <div style="display:flex; flex-direction:column; gap:12px; margin-top:32px">
                    ...
                    <?php if (!$isInModal): ?>
                        <a href="<?= base_url('index.php') ?>" style="color:var(--muted); font-size:14px; font-weight:600; text-decoration:none">
