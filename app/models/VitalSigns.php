...
final class VitalSigns
{
    ...
    {
        $stmt = db()->prepare("SELECT * FROM vital_signs WHERE patient_id = :pid ORDER BY recorded_at DESC LIMIT :limit");
    ...
    {
        $stmt = db()->prepare("SELECT weight_kg, recorded_at FROM vital_signs WHERE patient_id = :pid AND weight_kg IS NOT NULL ORDER BY recorded_at ASC LIMIT :limit");
        $stmt->execute([':pid' => $patientId, ':limit' => $limit]);
        return $stmt->fetchAll();
    }
